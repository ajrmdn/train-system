...
describe '#Train' do
  ...
  describe('.save') do
    it("creates and saves a new train to the database") do
      train = Train.new({:color => "red", :type => "freight", :id => 1})
      train.save
  ...
  describe('.all') do
    it("lists all trains in the database") do
      train1 = Train.new({:color => "red", :type => "freight", :id => 1})
      train1.save
      train2 = Train.new({:color => "blue", :type => "passanger", :id => 2})
      train2.save
      expect(Train.all).to(eq([train2, train1]))
    end
  end

  describe('.update') do
    it("updates information on a pre-existing train") do
      train = Train.new({:color => "red", :type => "freight", :id => nil})
      train.save
